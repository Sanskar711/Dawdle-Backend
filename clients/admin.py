...
class MeetingAdmin(admin.ModelAdmin):
    ...
    def get_meeting_qualifying_question_responses(self, obj):
        # Format the responses to display questions alongside answers
        responses = obj.qualifying_question_responses.all()
        formatted_responses = [
            f"{response.question.question_text}: {response.response}"
            for response in responses
        ]
        return format_html("<br>".join(formatted_responses))
